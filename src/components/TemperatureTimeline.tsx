...
export function TemperatureTimeline({ objectId, limit = 30 }: { objectId: string; limit?: number }) {
  ...
  if (events.length === 0) return <p className="text-sm text-muted-foreground">尚无温度变化记录。</p>;
  ...
  return (
    <ol className="space-y-3">
      {events.map((e) => {
        ...
        return (
          <li key={e.id} className="grid grid-cols-[110px_60px_1fr_70px] items-center gap-3 border-b border-border pb-3 text-sm">
            ...
            <span
              ...
              style={{ color: flat ? "var(--muted-foreground)" : up ? "var(--temp-hot)" : "var(--temp-cool)" }}
            >
              {up ? "+" : ""}{Number(e.delta).toFixed(1)}°
            </span>
