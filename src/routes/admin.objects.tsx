    ...
    <div className="container-prose py-12">
      ...
      <section className="mt-8 border border-border bg-card p-6">
        ...
        >
          ...
          <button className="border border-foreground bg-foreground px-4 py-2 text-sm text-background hover:bg-accent">
            创建
          </button>
          <textarea
            placeholder="描述（可选）"
            ...
            className="border border-border bg-background p-2.5 text-sm md:col-span-3"
            rows={2}
          />
        </form>
      </section>
      ...
      <section className="mt-10 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  wrap(
                    o.id,
                    () => updateCat({ data: { object_id: o.id, type: e.target.value as never } }),
                    "已更新类型",
                  )
                }
              ...
              {o.hidden && (
                <span className="border border-temp-warm px-1.5 py-0.5 text-[10px] text-temp-warm">
                  隐藏
                </span>
              )}
              {o.is_public_preview && (
              ...
              <span className="ml-auto text-sm tabular-nums">
                {Number(o.temperature).toFixed(0)}°
              </span>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              <button
                ...
                onClick={() =>
                  wrap(
                    o.id,
                    () => freeze({ data: { object_id: o.id, frozen: !o.frozen } }),
                    o.frozen ? "已解冻" : "已冻结",
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  wrap(
                    o.id,
                    () => hide({ data: { object_id: o.id, hidden: !o.hidden } }),
                    o.hidden ? "已显示" : "已隐藏",
                  )
                }
                className="border border-border px-3 py-1 text-xs hover:border-foreground"
              ...
              <button
                ...
                onClick={() =>
                  wrap(
                    ...
                    () =>
                      setPreview({
                        data: { object_id: o.id, is_public_preview: !o.is_public_preview },
                      }),
                    o.is_public_preview ? "已取消公开预览" : "已设为公开预览",
                  )
                }
                className="border border-border px-3 py-1 text-xs hover:border-foreground"
              >
              ...
              <button
                onClick={() => {
                  ...
                  wrap(
                    o.id,
                    () => recompute({ data: { object_id: o.id, manual_temperature: v } }),
                    "已设定",
                  );
              ...
              <select
                value={mergeTo[o.id] ?? ""}
                onChange={(e) => setMergeTo({ ...mergeTo, [o.id]: e.target.value })}
                className="border border-border bg-background px-2 py-1 text-xs"
              >
                <option value="">合并到…</option>
                {items
                  .filter((x) => x.id !== o.id && !x.merged_into)
                  .map((x) => (
              ...
              <button
                onClick={() => {
                  const t = mergeTo[o.id];
                  if (!t) return toast.error("选择目标对象");
        ...
        {items.length === 0 && (
          <p className="py-12 text-center text-sm text-muted-foreground">暂无对象</p>
        )}
      </section>
    </div>
  );
