...
function PrinciplesPanel() {
  ...
  const refresh = useCallback(() => list().then(setRows), [list]);
  useEffect(() => {
    refresh();
  }, [refresh]);
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between">
        ...
        <button
          onClick={() =>
            setEdit({ code: "", name: "", weight: 1, active: true, display_order: rows.length + 1 })
          }
            ...
            {rows.map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="p-3 text-center">
                  <button onClick={() => setEdit(p)} className="text-xs underline mr-3">
                    编辑
                  </button>
                  <button
                    onClick={async () => {
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Field label="顺序">
                <input
                  type="number"
                  value={edit.display_order ?? 0}
                  onChange={(e) => setEdit({ ...edit, display_order: +e.target.value })}
                  className="input"
                />
              </Field>
              <Field label="启用">
            ...
            <button
              onClick={async () => {
                try {
                  await upsert({ data: { ...edit, code: edit.code!, name: edit.name! } as never });
                  toast.success("已保存");
                  setEdit(null);
                  refresh();
                } catch (e) {
              ...
              className="w-full border border-foreground bg-foreground py-2 text-sm text-background"
            >
              保存
            </button>
          </div>
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between">
        ...
        <button
          onClick={() => setEdit({ code: "", name_zh: "", weight: 5, active: true })}
          className="border border-foreground px-3 py-1.5 text-sm hover:bg-foreground hover:text-background"
        >
          新增标签
        </button>
      ...
      <div className="border border-border">
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="p-3 text-left">code</th>
              <th className="p-3 text-left">中文</th>
            ...
            {rows.map((t) => (
              <tr key={t.id} className="border-t border-border">
                ...
                <td className="p-3 text-center text-xs">
                  {t.active ? (t.merged_into ? "已合并" : "活跃") : "停用"}
                </td>
                <td className="p-3 text-center">
                  <button onClick={() => setEdit(t)} className="text-xs underline mr-2">
                    编辑
                  </button>
                  <button
                    onClick={() => {
                  ...
                  <button
                    onClick={async () => {
                      await upsert({ data: { ...t, active: !t.active } as never });
                      refresh();
                    }}
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={async () => {
                try {
                  await upsert({
                    data: { ...edit, code: edit.code!, name_zh: edit.name_zh! } as never,
                  });
                  toast.success("已保存");
                  setEdit(null);
                  refresh();
      ...
      {mergeFrom && (
        <Modal onClose={() => setMergeFrom(null)} title={`合并：${mergeFrom.name_zh} → ？`}>
          <select value={mergeTo} onChange={(e) => setMergeTo(e.target.value)} className="input">
            ...
            {rows
              .filter((t) => t.id !== mergeFrom.id && t.active && !t.merged_into)
              .map((t) => (
          ...
          <button
            ...
            onClick={async () => {
              try {
                await merge({ data: { source_id: mergeFrom.id, target_id: mergeTo } });
                toast.success("已合并");
                setMergeFrom(null);
                refresh();
              } catch (e) {
            ...
            className="mt-3 w-full border border-foreground bg-foreground py-2 text-sm text-background disabled:opacity-50"
          >
            确认合并
          </button>
        </Modal>
...
function CasesPanel() {
  ...
  const refresh = useCallback(() => list({ data: {} }).then(setRows), [list]);
  useEffect(() => {
    refresh();
    tagsFn().then(setAllTags);
    principlesFn().then(setAllPrinciples);
  }, [refresh, tagsFn, principlesFn]);
    ...
    <div>
          ...
          <div key={c.id} className="border border-border p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                {c.tags.length > 0 && (
                  <p className="mt-2 text-xs text-muted-foreground">标签：{c.tags.join("、")}</p>
                )}
                {c.principles.length > 0 && (
                  <p className="text-xs text-muted-foreground">原则：{c.principles.join("、")}</p>
                )}
              </div>
              <div className="flex flex-col gap-1 text-xs">
                <button onClick={() => setEdit(c)} className="underline">
                  编辑
                </button>
                <button
                  onClick={async () => {
      ...
      {edit && (
        <Modal
          ...
          title={edit.id ? `编辑案例 ${edit.code ?? ""}` : "新增案例"}
        >
          <div className="space-y-3">
            <Field label="标题">
              <input
            ...
            <div className="grid grid-cols-3 gap-3">
              <Field label="性质">
                <select
                  value={edit.polarity ?? "negative"}
                  onChange={(e) => setEdit({ ...edit, polarity: e.target.value as never })}
                  className="input"
                >
              ...
              <Field label="状态">
                <select
                  ...
                  onChange={(e) => setEdit({ ...edit, status: e.target.value as never })}
                  className="input"
                >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEdit({
                          ...edit,
                          principles: on
                            ? (edit.principles ?? []).filter((x) => x !== p.code)
                            : [...(edit.principles ?? []), p.code],
                        })
                      }
                      className={`px-2 py-1 text-xs border ${on ? "border-foreground bg-foreground text-background" : "border-border"}`}
                    >
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="bg-background w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-border p-6"
