...
import { useAdminUnread } from "@/hooks/use-admin-unread";
...
type NavItem = {
  to: string;
  label: string;
  unreadKey?: "comments" | "feedback" | "objects";
};
...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: "/admin/objects", label: "对象", unreadKey: "objects" },
  { to: "/admin/observations", label: "观察审核" },
  { to: "/admin/comments", label: "留言", unreadKey: "comments" },
  { to: "/admin/feedback", label: "建议", unreadKey: "feedback" },
...
function AdminLayout() {
  ...
  const { unread, markViewed } = useAdminUnread();
        ...
        <div className="container-prose flex items-center gap-1 overflow-x-auto py-3 text-sm">
          ...
          {NAV_ITEMS.map((t) => (
            <Link
              ...
              onClick={() => {
                if (t.unreadKey) markViewed(t.unreadKey);
              }}
              className="inline-flex items-center gap-1.5 rounded-sm px-3 py-1.5 text-muted-foreground hover:text-foreground"
            >
              {t.label}
              {t.unreadKey && unread[t.unreadKey] > 0 && (
                <Badge variant="destructive" className="px-1.5 py-0 text-[10px] leading-none">
                  {unread[t.unreadKey]}
                </Badge>
              )}
            </Link>
