...
function Audit() {
  ...
  return (
    <div className="container-prose py-12">
        ...
        <tbody>
          {logs.map((l) => (
            <tr key={l.id} className="border-b border-border align-top">
              ...
              <td>
                <span className="border border-border px-1.5 py-0.5 text-xs">{l.action}</span>
              </td>
      ...
      {logs.length === 0 && (
        <p className="py-12 text-center text-sm text-muted-foreground">暂无日志</p>
      )}
    </div>
  );
}
