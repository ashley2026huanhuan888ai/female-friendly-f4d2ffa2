...
export const getHomeSummary = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    const pack = (ids: string[]) => ids.map((id) => {
      const o = oMap.get(id);
...
export const getTrendingTopics = createServerFn({ method: "GET" })
  .inputValidator((i) => z.object({ days: z.number().int().min(1).max(90).default(30).optional() }).partial().parse(i ?? {}))
  .handler(async ({ data }) => {
...
export const followObject = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await supabaseAdmin.from("watchlist" as never)
...
export const unfollowObject = createServerFn({ method: "POST" })
  ...
  .inputValidator((i) => z.object({ object_id: z.string().uuid() }).parse(i))
  .handler(async ({ data, context }) => {
    await supabaseAdmin.from("watchlist" as never).delete()
      .eq("user_id", context.userId).eq("object_id", data.object_id);
    return { ok: true };
  });
...
export const isFollowing = createServerFn({ method: "GET" })
  ...
  .inputValidator((i) => z.object({ object_id: z.string().uuid() }).parse(i))
  .handler(async ({ data, context }) => {
    const { data: row } = await supabaseAdmin.from("watchlist" as never)
      .select("id").eq("user_id", context.userId).eq("object_id", data.object_id).maybeSingle();
...
export const getMyWatchlist = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data: rows } = await supabaseAdmin.from("watchlist" as never)
      .select("object_id, created_at").eq("user_id", context.userId)
      .order("created_at", { ascending: false });
    const ids = (rows ?? []).map((r: { object_id: string }) => r.object_id);
    if (ids.length === 0) return [];
    const { data: objs } = await supabaseAdmin.from("objects")
      .select("id, name, type, temperature, observation_count, ai_summary").in("id", ids);
    return objs ?? [];
  });
...
export const getMyDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const [watch, myObs, notif] = await Promise.all([
      supabaseAdmin.from("watchlist" as never).select("object_id").eq("user_id", context.userId),
      supabaseAdmin.from("observations")
        .select("id, object_id, status, summary, created_at")
        .eq("user_id", context.userId)
        .order("created_at", { ascending: false }).limit(20),
      supabaseAdmin.from("notifications" as never)
    ...
    const oIds = [...new Set([
      ...
      ...((notif.data ?? []).map((n: { object_id: string | null }) => n.object_id).filter(Boolean) as string[]),
    ])];
    ...
    return {
      ...
      my_observations: (myObs.data ?? []).map((o) => ({ ...o, object: oMap.get(o.object_id) ?? null })),
      notifications: (notif.data ?? []).map((n: { object_id: string | null }) => ({
        ...n, object: n.object_id ? oMap.get(n.object_id) ?? null : null,
      })),
...
export const markNotificationsRead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    let q = supabaseAdmin.from("notifications" as never)
