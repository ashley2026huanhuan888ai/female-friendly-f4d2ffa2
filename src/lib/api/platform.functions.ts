...
// ===== 管理员未读计数（自上次查看以来的新记录数）=====
export const getAdminUnreadCounts = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) =>
    z
      .object({
        comments_since: z.string().optional().default(new Date(0).toISOString()),
        feedback_since: z.string().optional().default(new Date(0).toISOString()),
        objects_since: z.string().optional().default(new Date(0).toISOString()),
      })
      .parse(input ?? {}),
  )
  .handler(async ({ context, data }) => {
    ...
    const [c, f, o] = await Promise.all([
      supabaseAdmin
        .from("object_comments" as never)
        .select("*", { count: "exact", head: true })
        .gt("created_at", data.comments_since),
      supabaseAdmin
        .from("platform_feedback" as never)
        .select("*", { count: "exact", head: true })
        .gt("created_at", data.feedback_since),
      supabaseAdmin
        .from("objects")
        .select("*", { count: "exact", head: true })
        .gt("created_at", data.objects_since),
    ]);

    return {
      comments_unread: c.count ?? 0,
      feedback_unread: f.count ?? 0,
      objects_unread: o.count ?? 0,
    };
  });
