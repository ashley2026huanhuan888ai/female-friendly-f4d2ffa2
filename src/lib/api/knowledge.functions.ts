...
// ===== 公共读取 =====
export const listPrinciples = createServerFn({ method: "GET" }).handler(async () => {
  const { data } = await supabaseAdmin
...
// ===== 知识库概览 =====
export const getKnowledgeOverview = createServerFn({ method: "GET" }).handler(async () => {
  const [p, t, c] = await Promise.all([
    ...
    supabaseAdmin
      .from("knowledge_cases" as never)
      .select("polarity, status", { count: "exact" })
      .eq("status", "published"),
  ]);
