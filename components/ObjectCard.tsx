...
export function ObjectCard({
  id, name, type, temperature, observation_count,
  ai_summary, top_tags, heat_sources, cooling_sources, updated_at,
  showActions = false,
}: Props) {
  ...
  return (
    <article className="group relative border border-border bg-card p-6 transition-all hover:border-foreground/40 hover:shadow-sm focus-within:border-foreground/60">
      {/* full-card click target — sibling of content, no <a> nesting */}
      <Link
        to="/objects/$id"
        params={{ id }}
        aria-label={`查看对象详情：${name}`}
        className="absolute inset-0 z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      />

      <div className="pointer-events-none relative z-20 flex items-start justify-between gap-6">
        <div className="min-w-0 flex-1">
          ...
          {tags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {tags.map((t) => (
                <span key={t.tag} className="border border-border px-2 py-0.5 text-[11px] text-muted-foreground">
                  #{t.tag}
                </span>
              ))}
            </div>
          ...
          {(heatLabel || coolLabel) && (
            <div className="mt-3 space-y-0.5 text-[11px] text-muted-foreground">
              {heatLabel && <div><span className="text-[var(--temp-hot,#c1440e)]">▲</span> 主要热源：{heatLabel}</div>}
              {coolLabel && <div><span className="text-[var(--temp-cool,#2563eb)]">▼</span> 主要降温：{coolLabel}</div>}
            </div>
          )}
      ...
      {showActions && (
        <div className="pointer-events-auto relative z-30 mt-5 flex flex-wrap items-center gap-2 border-t border-border pt-4">
          <Link
            to="/submit/$objectId"
            params={{ objectId: id }}
