...
function SubmitPage() {
  ...
  const retryAnalysis = async () => {
    ...
    try {
      const [res] = await Promise.all([
        retrySavedAnalysis({ data: { id: savedId } }),
        minWait,
      ]);
      clearInterval(stepTimer);
      setStepIdx(STEPS.length - 1);
  ...
  // 错误页面（AI/网络失败但可能已保存）
  if (phase === "error") {
    const savedButAiFailed = !!result?.ai_failed;
    return (
      <SiteLayout>
        <div className="container-prose max-w-2xl py-20">
          <div className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">提示</div>
          <h1 className="mt-3 font-serif text-2xl">
            {savedButAiFailed ? "观察已保存，AI 分析暂时不可用" : "提交失败"}
          </h1>
          <p className="mt-3 text-sm text-muted-foreground">
            {savedButAiFailed
              ? "你的观察已保存为待审。AI 分析失败，管理员可以在后台重新分析；你也可以稍后重试 AI 分析。"
              : "网络或服务器异常，观察未能保存。请重试。"}
          </p>
          {errorMsg && (
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              className="border border-foreground bg-foreground px-4 py-2 text-xs uppercase tracking-wider text-background hover:bg-accent hover:border-accent"
            >
              {savedButAiFailed ? "重试 AI 分析" : "一键重试分析"}
            </button>
            <button
